...
st.set_page_config(page_title="TQQQ / 코코레 QUANT V5 DAILY", page_icon="📈", layout="centered")

st.title("📈 TQQQ / 코코레 QUANT V5 — 일봉 전용")
st.caption("일봉 전용: 현금 대기 → 종가 기준 낙폭 분할매수 → 목표수익 익절 → 재대기")
...
st.info(
    "이 버전은 분봉을 사용하지 않습니다. 모든 백테스트·매수 신호·익절 판단은 일봉 종가 기준입니다. "
    "현금을 보유하다가 하락 구간에서 분할매수하고 목표수익률에 도달하면 전량 익절합니다."
)
...
def get_data(asset, ref):
    x = yf.download([asset, ref], period="max", interval="1d", auto_adjust=True, progress=False)["Close"]
...
try:
    ...
    st.caption("※ 순위는 최종자산을 최우선으로 정렬합니다. 모든 계산은 일봉 종가 기준이며 MDD·승률은 참고용입니다.")
    ...
    st.caption(
        "현재 행동 값은 최근 일봉 종가와 선택된 고점 기준으로 계산한 참고값입니다. "
        "분봉·실시간 장중 가격·주문 체결은 반영하지 않습니다."
    )
    ...
    st.divider()
    st.caption(
        "백테스트는 과거 일봉 종가 기준 단순 시뮬레이션입니다. 세금, 수수료, 슬리피지, 환율, "
