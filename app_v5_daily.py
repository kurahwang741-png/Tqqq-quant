...
def _trade_market_key(row):
    market_value = str(row.get("시장", "")).strip().upper()
    symbol_value = str(row.get("종목", "")).strip().upper()
    if symbol_value == "SOXL" or market_value == "SOXL":
        return "SOXL"
    return ""

def _is_market_cycle_locked(records, market_key="SOXL"):
    if not records:
        return False
...
active_market_key = "SOXL"
cycle_locked_for_ui = _is_market_cycle_locked(st.session_state.get("live_trades", []))

if cycle_locked_for_ui:
    st.info("🔒 SOXL 현재 사이클 운용 중 — 전량매도까지 현재 전략을 고정합니다.")
